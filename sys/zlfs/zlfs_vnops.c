...
zlfs_lookup(void *v)
{
	...
	*ap->a_vpp = NULL;
	...
	if ((cnp->cn_flags & ISDOTDOT) ||
		...
		/*
		 * The synthetic root is its own parent; the VFS layer
		 * handles ".." crossing out of the filesystem root.
		 */
		vref(dvp);
		*ap->a_vpp = dvp;
		return 0;
	...
	/* No other names exist; a create/rename here fails read-only. */
	if ((cnp->cn_nameiop == CREATE || cnp->cn_nameiop == DELETE ||
	    cnp->cn_nameiop == RENAME) && (cnp->cn_flags & ISLASTCN))
		return EROFS;
...
/*
 * Emit "." and ".." for the synthetic empty root.  uio_offset counts
 * directory entries, not bytes.
 */
int
zlfs_readdir(void *v)
{
	...
	*ap->a_eofflag = 0;
	...
	for (i = uio->uio_offset; i < 2; i++) {
		memset(&d, 0, sizeof(d));
		d.d_fileno = znp->zn_ino;
		d.d_type = DT_DIR;
		d.d_namlen = i + 1;
...
zlfs_reclaim(void *v)
{
	...
	if (znp != NULL) {
		if (znp->zn_zmp != NULL && znp->zn_zmp->zm_rootvp == vp)
			znp->zn_zmp->zm_rootvp = NULL;
		cache_purge(vp);
