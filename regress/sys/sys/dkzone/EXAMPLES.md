...
refuses ordinary non-zoned `sd(4)` disks, verifies that the raw disk reports
host-managed `zone_mode=4`, and then runs `dkzone-vm-smoke.sh` unchanged.  A
passing run should end with the same final `ok` as the NVMe ZNS transcript
below.
...
If the target evidence says `sdX: ...` without `zoned`, the wrapper should
stop before the build and mutating tests.  That is the expected result for the
normal VirtIO boot disk.
